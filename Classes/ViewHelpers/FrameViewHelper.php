...
class FrameViewHelper extends AbstractViewHelper
{
    ...
    public function initializeArguments()
    {
        ...
        $this->registerArgument('backgroundMedia', 'mixed', 'media object or src');
    ...
    public static function renderStatic(
        ...
        $backgroundMediaId = 'frame-backgroundmedia-' . $identifier;
        ...
        // var_dump($configuration['backgroundMedia']);
        \TYPO3\CMS\Core\Utility\DebugUtility::debug($configuration['backgroundMedia']);


        if (is_object($configuration['backgroundMedia']) && ($configuration['backgroundMedia'] instanceof FileReference || $configuration['backgroundMedia'] instanceof File)) {
            $backgroundMediaFile = $configuration['backgroundMedia'];
        } elseif (is_string($configuration['backgroundMedia'])) {
            $resourceFactory = GeneralUtility::makeInstance(ResourceFactory::class);
            $backgroundMediaAbsFilename = GeneralUtility::getFileAbsFileName($configuration['backgroundMedia']);
            if (file_exists($backgroundMediaAbsFilename)) {
                // Do not use absolute file name to ensure correct path resolution from ResourceFactory.
                $backgroundMediaFile = $resourceFactory->retrieveFileOrFolderObject($configuration['backgroundMedia']);
            }
        }
        ...
        if ($backgroundMediaFile !== null) {
            $classes[] = 'frame-has-backgroundmedia';

            // var_dump($backgroundMediaFile->getIdentifier());
            // \TYPO3\CMS\Core\Utility\DebugUtility::debug($backgroundMediaFile);

        } else {
            $classes[] = 'frame-no-backgroundmedia';
        }
    ...
    protected static function getTemplateObject(): StandaloneView
    {
        ...
        $layoutRootPaths = [];
        $layoutRootPaths[] = GeneralUtility::getFileAbsFileName('EXT:videoprocessing/Resources/Private/Layouts/ViewHelpers/');
        if (isset($setup['plugin.']['tx_videoprocessing.']['view.']['layoutRootPaths.'])) {
            foreach ($setup['plugin.']['tx_videoprocessing.']['view.']['layoutRootPaths.'] as $layoutRootPath) {
        ...
        if (isset($setup['plugin.']['tx_videoprocessing.']['view.']['partialRootPaths.'])) {
            foreach ($setup['plugin.']['tx_videoprocessing.']['view.']['partialRootPaths.'] as $partialRootPath) {
        ...
        if (isset($setup['plugin.']['tx_videoprocessing.']['view.']['templateRootPaths.'])) {
            foreach ($setup['plugin.']['tx_videoprocessing.']['view.']['templateRootPaths.'] as $templateRootPath) {
